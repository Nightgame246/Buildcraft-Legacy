...
This project is developed using multiple AI coding assistants in the terminal:

- **Claude Code** (`claude`) — complex logic, architecture decisions, difficult bugs
- **Gemini CLI** (`gemini`) — large context analysis (1M token window), reading entire subsystems at once
- **Qwen2.5-Coder 32B** (via LM Studio at `localhost:1234`) — routine porting, boilerplate, test generation

When analyzing a large BuildCraft subsystem, feed the original 1.12 source to Gemini first to get a migration plan, then implement class by class using Claude or the local model.
